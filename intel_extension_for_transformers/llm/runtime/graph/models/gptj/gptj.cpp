...
static bool gptj_model_eval_internal(model_context& lctx, const model_token* tokens, const int n_tokens,
  ...
  for (int il = 0; il < n_layer; ++il) {
    ...
    if (run_mha_reordered) {  // reordered kv-cache bf16 mha must be used if run_mha_reordered
      ...
      struct ne_tensor* KQV_Out = ne_flash_attn(ctx0, Q, K, V, 1.0f / sqrtf(float(head_size)), n_past == 0);
      KQV_merged_contiguous = ne_view_2d(ctx0, KQV_Out, head_size * n_head, N * batch_size, head_size * n_head * ne_element_size(KQV_Out), 0);
      ...
      struct ne_tensor* KQV_Out = ne_flash_attn(ctx0, Q, K, Vtmp, 1.0f / sqrtf(float(head_size)), n_past == 0);
      KQV_merged_contiguous = ne_view_2d(ctx0, KQV_Out, head_size * n_head, N * batch_size, head_size * n_head * ne_element_size(KQV_Out), 0);
    } else {
      // K * Q
